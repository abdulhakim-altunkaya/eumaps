document.addEventListener("DOMContentLoaded", () => {
  ...
  function calculateTax(event) {
    ...
    if (!selectedCondition) {
      showMessage("Motosikletin yeni veya ikinci el olduğunu seçin.");
      return;
    }
    ...
    if (
      ...
      setInputError(
        productionYearInput,
        "Motosikletin üretim yılını dört rakam olarak girin."
      );
      return;
    }
    ...
    if (invoiceYear < productionYear) {
      showMessage("Fatura yılı, motosikletin üretim yılından önce olamaz.");
      return;
    }

    if (invoiceYear - productionYear > 3) {
      showMessage(
        "Motosiklet satın alındığı tarihte üç yaşından büyük olmamalıdır. Bu motosiklet ithal edilemez."
      );
      return;
    }
    ...
    if (selectedCondition.value === "new") {
      if (invoiceYear - productionYear >= 1) {
        ...
        vehicleWarning.textContent =
          "Üretim yılı ile fatura yılı farklı olduğu için motosiklet ikinci el olarak değerlendirildi.";
    ...
    const basePriceLira = basePrice * currencyRate;

    if (!Number.isFinite(basePriceLira)) {
      showMessage("Kur dönüşümü sırasında hesaplama hatası oluştu.");
      return;
    }

    const taxPercentage =
      engineCapacity < 251 ? 0.08 : 0.37;
