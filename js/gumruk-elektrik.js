document.addEventListener("DOMContentLoaded", () => {
  ...
  function calculateTax(event) {
    ...
    const enginePower = Number(enginePowerInput.value);
    ...
    if (
      enginePowerInput.value.trim() === "" ||
      !Number.isFinite(enginePower) ||
      enginePower < 10 ||
      enginePower > 10000
    ) {
      setInputError(
        enginePowerInput,
        "Motor gücünü 10 ile 10.000 kW arasında girin."
      );
      return;
    }
    ...
    const taxPercentage = getElectricTaxPercentage(
      enginePower,
      basePriceLira
    );
  ...
  function getElectricTaxPercentage(enginePower, basePriceLira) {
    if (enginePower < 161 && basePriceLira < 1650001) {
      return 0.25;
    }
    ...
    if (enginePower > 160 && basePriceLira < 1650001) {
      return 0.65;
    }
