...
router.get("/api/get/grills-latvia/poster-profile-picture/:gid", applyReadRateLimit, async (req, res) => {
    try {
...
router.get("/api/get/grills-latvia/poster-likes/:gid", applyReadRateLimit, async (req, res) => {
    try {
      const gid = req.params.gid;
      if (!gid) {
        return res.status(400).json({
...
router.get("/api/get/grills-latvia/poster-stats/:gid", applyReadRateLimit, async (req, res) => {
  try {
    const gid = req.params.gid;
    if (!gid) {
      return res.status(400).json({
    ...
    const posterName = userRes.rows[0].name || null;
